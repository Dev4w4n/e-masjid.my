 ...
 */
const TIER_PACKAGES: Record<TierId, TierPackage> = {
  asas: {
    id: "asas",
    name_ms: "Asas",
    name_en: "Asas (Free)",
    description_ms:
      "Percuma, tanpa pendaftaran. Sempurna untuk masjid kecil. Paparkan waktu solat JAKIM di TV anda tanpa bayaran selamanya.",
    description_en:
      "Free, no sign-up required. Perfect for small mosques. Display JAKIM prayer times on your TV at no cost forever.",
    price_ms: "Percuma",
    price_en: "Free",
    max_screens: 1,
  ...
  maju: {
    ...
    name_en: "Maju",
    description_ms:
      "Perkhidmatan terurus. Kami mengendalikan pembaruan kandungan melalui WhatsApp. Ideal untuk masjid yang mahir teknologi sederhana.",
    description_en:
      "Managed service. We handle content updates via WhatsApp. Ideal for tech-savvy mosques with basic needs.",
  ...
  gemilang: {
    ...
    name_en: "Gemilang",
    description_ms:
      "Kawalan sendiri. Papan pemuka pentadbir untuk masjid sederhana hingga besar. Berbilang skrin, tiada had.",
    description_en:
      "Self-service. Admin dashboard for medium to large mosques. Multiple screens, unlimited customization.",
  ...
  istimewa: {
    ...
    name_en: "Istimewa",
    description_ms:
      "Perusahaan. Penyepaduan API tersuai, berbilang cawangan, sokongan khusus. Untuk organisasi besar dan kompleks.",
    description_en:
      "Enterprise. Custom API integration, multi-branch support, dedicated account manager. For large organizations.",
...
export class TierPackageService implements ITierPackageService {
   ...
   */
  async fetchAllTiers(): Promise<TierPackage[]> {
    try {
      ...
      const tiers: TierPackage[] = Object.values(TIER_PACKAGES).sort(
   ...
   */
  async fetchTierById(tierId: TierId): Promise<TierPackage | null> {
    try {
      // Validate tierId
      if (!["asas", "maju", "gemilang", "istimewa"].includes(tierId)) {
        throw new ServiceError(
          ServiceErrorCode.TIER_NOT_FOUND,
   ...
   */
  async fetchTiersForLanding(): Promise<TierPackage[]> {
    try {
      ...
      throw new ServiceError(
        ServiceErrorCode.SERVICE_UNAVAILABLE,
        `Failed to fetch landing page tiers: ${error}`,
        500,
        { error },
      );
    }
  ...
  async compareTiers(
    tierIds: TierId[],
  ): Promise<Map<string, Record<TierId, unknown>>> {
    try {
      const tiers = await Promise.all(
        tierIds.map((id) => this.fetchTierById(id)),
      );

      const comparison = new Map<string, Record<TierId, unknown>>();
      ...
      for (const dimension of dimensions) {
        ...
        for (let i = 0; i < tierIds.length; i++) {
          ...
          if (tier && tierId) {
            dimensionData[tierId] = tier[dimension as keyof TierPackage];
          }
        }
        comparison.set(dimension, dimensionData as Record<TierId, unknown>);
      }
