...
export function formatAddressSingleLine(address: ProfileAddress | MasjidAddress): string {
  return formatAddress(address);
}

// Format address for multi-line display
export function formatAddressMultiLine(address: ProfileAddress | MasjidAddress): string[] {
  const lines: string[] = [];
...
export function guessStateFromPostcode(postcode: string): MalaysianState | null {
...
export function isPostcodeValidForState(postcode: string, state: MalaysianState): boolean {
...
export const STATE_CITIES: Record<MalaysianState, string[]> = {
  'Johor': ['Johor Bahru', 'Muar', 'Batu Pahat', 'Kluang', 'Pontian', 'Segamat', 'Kulai'],
  ...
  'Negeri Sembilan': ['Seremban', 'Port Dickson', 'Nilai', 'Rembau', 'Kuala Pilah'],
  ...
  'Perak': ['Ipoh', 'Taiping', 'Sitiawan', 'Kuala Kangsar', 'Teluk Intan', 'Parit Buntar'],
...
export function isValidCityForState(city: string, state: MalaysianState): boolean {
  const cities = STATE_CITIES[state];
  if (!cities) return true; // Allow if we don't have city data
  
  const normalizedCity = city.toLowerCase();
  return cities.some(validCity => validCity.toLowerCase().includes(normalizedCity));
}

// Convert ProfileAddress to MasjidAddress format
export function profileAddressToMasjidAddress(address: ProfileAddress): MasjidAddress {
