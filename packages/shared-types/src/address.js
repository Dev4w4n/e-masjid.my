...
export function normalizeStateName(state) {
    // Direct match
    if (Object.values(STATE_MAPPING).includes(state)) {
        return state;
    }
    // Check mapping
...
export const POSTCODE_TO_STATE = {
    // Kuala Lumpur
    '50': 'Kuala Lumpur',
    '51': 'Kuala Lumpur',
    '52': 'Kuala Lumpur',
...
export function guessStateFromPostcode(postcode) {
...
export function isValidCityForState(city, state) {
    const cities = STATE_CITIES[state];
    if (!cities)
        return true; // Allow if we don't have city data
...
export function profileAddressToMasjidAddress(address) {
    const result = {
...
export function masjidAddressToProfileAddress(address, profileId, addressType = 'home') {
    return {
