...
export function MasjidCard({ 
  ...
  const formatAddress = () => {
    try {
      ...
      const address = typeof masjid.address === 'string' 
      ...
      <CardContent>
        <Box sx={{ display: 'flex', alignItems: 'center', mb: 2 }}>
          ...
          <Box sx={{ flex: 1 }}>
            ...
            <Chip
              label={masjid.status === 'active' ? 'Active' : masjid.status === 'inactive' ? 'Inactive' : 'Pending'}
...
interface AdminAssignmentCardProps {
  assignment: Database['public']['Tables']['masjid_admins']['Row'] & {
    profiles: Pick<Database['public']['Tables']['profiles']['Row'], 
      'user_id' | 'full_name' | 'phone_number'> | null;
  };
  onRemove?: () => void;
  showActions?: boolean;
}
  ...
  return (
    <Card>
      <CardContent>
        ...
        <Box sx={{ display: 'flex', alignItems: 'center', mb: 1 }}>
          <AdminPanelSettings sx={{ mr: 1, fontSize: 16, color: 'text.secondary' }} />
          <Typography variant="body2">
