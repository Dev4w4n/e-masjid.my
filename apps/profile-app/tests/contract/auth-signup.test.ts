/**
 * Contract Test: POST /auth/sign-up
 * 
...
describe('POST /auth/sign-up - User Registration Contract', () => {
  const validTestUser = {
  ...
  describe('Successful Registration (201)', () => {
    it('should register a new user with valid email and password', async () => {
      ...
      expect(data.user.role).toBe('registered'); // Default role for new users
    ...
    it('should create user with proper role assignment', async () => {
      ...
      // New users should start with 'registered' role
      expect(data.user.role).toBe('registered');
      expect(data.user.email_verified).toBe(false); // Should require email verification
    });
  });
  ...
  describe('Conflict Errors (409)', () => {
    it('should reject registration with existing email', async () => {
      ...
      const firstResponse = await fetch(`${API_BASE_URL}/auth/sign-up`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json'
        },
      ...
      const secondResponse = await fetch(`${API_BASE_URL}/auth/sign-up`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json'
        },
