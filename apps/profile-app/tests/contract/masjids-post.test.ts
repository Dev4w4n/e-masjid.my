...
describe('POST /masjids - Masjid Creation Contract', () => {
  ...
  const masjidAdminCredentials = {
    email: 'masjid.admin@example.com',
  ...
  beforeAll(async () => {
    ...
    // Create regular user
    const regularUserSignUp = await fetch(`${API_BASE_URL}/auth/sign-up`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json'
      },
    ...
    regularUserToken = regularUserData.session.access_token;

    // Create masjid admin user
    const masjidAdminSignUp = await fetch(`${API_BASE_URL}/auth/sign-up`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json'
      },
    ...
    masjidAdminToken = masjidAdminData.session.access_token;
  ...
  describe('Successful Masjid Creation (201) - Super Admin Only', () => {
    it('should create masjid with all fields provided', async () => {
      ...
      expect(masjid.registration_number).toBe(validMasjidData.registration_number);
      ...
      expect(masjid.address.address_line_1).toBe(validMasjidData.address.address_line_1);
      expect(masjid.address.address_line_2).toBe(validMasjidData.address.address_line_2);
    ...
    it('should create masjid with minimal required fields', async () => {
      ...
      expect(masjid.address.address_line_1).toBe(minimalMasjidData.address.address_line_1);
    ...
    it('should generate unique IDs for different masjids', async () => {
      ...
      const response1 = await fetch(`${API_BASE_URL}/masjids`, {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${superAdminToken}`,
          'Content-Type': 'application/json'
      ...
      const response2 = await fetch(`${API_BASE_URL}/masjids`, {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${superAdminToken}`,
          'Content-Type': 'application/json'
  ...
  describe('Validation Errors (400)', () => {
    it('should reject masjid with missing required fields', async () => {
      const incompleteData = [
        ...
        { ...validMasjidData, address: { ...validMasjidData.address, address_line_1: undefined } },
        { ...validMasjidData, address: { ...validMasjidData.address, city: undefined } },
        { ...validMasjidData, address: { ...validMasjidData.address, state: undefined } },
        { ...validMasjidData, address: { ...validMasjidData.address, postcode: undefined } }
      ];
